...
export default {
    data: () => ({
        ...
        items: [
            ...
            {
                text: "Participación como Jurado",
                disabled: true,
                href: "/dashboard/project/jury"
            }
        ],
        jurys: [],
        editedIndex: -1,
    ...
    methods: {
            ...
            this.jurys = [
                {
                    id: 0,
                    title:
                        "APLICATIVO MOVIL PARA LA GESTION DE LOS PROCESOS JUDICIALES EN UNA OFICINA DE ABOGADOS INDEPENDIENTES.",
                    created_at: "2019-10-11 03:09:46",
                    updated_at: "2019-10-11 03:09:46",
                    year: "2017-01",
                    entity: "Universidad Popular del Cesar. ",
                    people:
                        "Kevin Luis Florez Lozada y Ricardo Andrés Sanchez Atuesta."
                }
            ];
            this.axios
                .get("/api/jury")
        ...
        editItem(item) {
            this.editedIndex = this.jurys.indexOf(item);
            this.editedItem = Object.assign({}, item);
            this.dialog = true;
        },

        deleteItem(item) {
            const index = this.jurys.indexOf(item);
            Swal.fire({
                title: "Estás seguro?",
        ...
        save() {
            if (this.validate()) {
                if (this.editedIndex > -1) {
                    ...
                    const juryNuevo = this.editedItem;
                    axios
                        .post("/api/jury", juryNuevo)
                        .then(res => {
                            const juryServidor = res.data;
                            this.jurys.push(juryServidor);
                            Swal.fire(
                                "Buen trabajo! 😃",
                                "Registro Exitoso! ",
                                "success"
        ...
        edit() {
            ...
            axios
                .put(`/api/jury/${this.jurys[index].id}`, params)
                .then(res => {
                    Object.assign(this.jurys[index], item);
                    Swal.fire(
                        "Buen trabajo!",
